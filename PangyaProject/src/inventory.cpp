...
void Inventory::add_transaction(uint8 types, std::shared_ptr<INV_ITEM> const& pitem_, uint32 old_amount) {
	assert(pitem_);
	std::shared_ptr<INV_TRANSACTION> tran(new INV_TRANSACTION());
	tran->item_typeid = pitem_->item_typeid;
	tran->item_id = pitem_->id;
	tran->old_amount = old_amount;
	tran->new_amount = pitem_->c0;
	transaction_.push_back(tran);
}
